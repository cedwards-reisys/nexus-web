...
class vBSphinxSearch_Core extends vB_Search_Core
{
	...
	private function connect($config)
	{
		...
		$dbClass = 'vB_Database_' . ucfirst(strtolower($config['Database']['dbtype']));
		$this->sphinxDB = new $dbClass();
...
// 					$this->where[''] = "(" . implode(' OR ', $qbits) . ")";
	...
	protected function process_my_channels_filter($params)
	{
		...
		ini_set('error_log', 'C:\xampp\htdocs\sprintXm17\errorlog.txt');
		error_log(__CLASS__ . "->" . __FUNCTION__ . "() @ LINE " . __LINE__
			. "\n" . "this->where:" . print_r($this->where, true)
		);
