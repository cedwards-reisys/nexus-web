...
class vB_WysiwygHtmlParser
{
	...
	*/
	...
	{
		$result = preg_replace('#<a href="([^"]*)\[([^"]+)"(.*)>(.*)\[\\2</a>#siU', // check for the WYSIWYG editor being lame with URL tags followed by bbcode tags
		...
		return preg_replace_callback('#(<[^<>]+ (src|href))=(\'|"|)??(.*)(\\3)#siU'  // make < and > safe in inside URL/IMG tags so they don't get stripped by strip_tags
			, array($this, 'escapeWithinUrlCallback'), $text
		);
	}

	/**
	 * Callback for preg_replace_callback in filter_html_tags
	 */
	protected function escapeWithinUrlCallback($matches)
	{
		return $this->escape_within_url($matches[1], $matches[4], $matches[3]);
	}
	...
	*/
	protected function filter_bbcode($text)
	{
		$text = preg_replace_callback('#\[(html|php)\]((?>[^\[]+?|(?R)|.))*\[/\\1\]#siU',				// strip html from php tags
			array($this, 'stripHtmlFromBbcodeCallback'), $text
		);
	...
	/**
	 * Callback for preg_replace_callback in filter_bbcode
	 */
	protected function stripHtmlFromBbcodeCallback($matches)
	{
		return $this->strip_html_from_bbcode($matches[0]);
	}
	...
	*/
	protected function parse_unmatched_tags($text)
	{
		...
		$text = preg_replace_callback('#<img[^>]+smilieid="(\d+)".*>#siU',
			array($this, 'translateSmilieIdTextCallback'), $text
		);
	...
	/**
	 * Callback for preg_replace_callback in parse_unmatched_tags
	 */
	protected function translateSmilieIdTextCallback($matches)
	{
		return $this->translate_smilie_id_text($matches[1]);
	}
