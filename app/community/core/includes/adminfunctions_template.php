...
function print_style($styleid, $style = '')
{
	...
	$styleIsReadonly = (!$vb5_config['Misc']['debug'] AND !empty($style['guid']));
	...
	if ($userContext->hasAdminPermission('canadminstyles'))
	{
		if (!$styleIsReadonly)
		{
			echo "	<option value=\"stylevar\" selected=\"selected\">$vbphrase[stylevareditor]</option>
...
function build_special_templates($newtemplates, $templatetype, $vartype)
{
	...
	foreach ($template_cache["$templatetype"] AS $title => $oldtemplate)
	{
		...
		if ($newtemplate != $oldtemplate['template'])
		{
			...
			{
				/*insert query*/
				vB::getDbAssertor()->update(	// todo this can't be right.
						'template',
						array(
								'title' => $title,
								'templatetype' => $templatetype,
