...
class vB_Upgrade_final extends vB_Upgrade_Version
{
	...
	function step_11()
	{
		$this->show_message($this->phrase['final']['creating_new_routes']);
		$contenttype = $this->db->query_first("
			SELECT contenttypeid
			FROM " . TABLE_PREFIX . "contenttype
			WHERE class = 'Channel'
		...
		while ($channel = array_shift($queue))
		{
			...
			$duplicateRegex = $this->db->query_first("
				...
				WHERE regex = '" . $this->db->escape_string($newprefix) . "'
			");
			...
			// @TODO the regex below and the regex used in vB5_Route_Channel
			// should be a constant in the vB5_Route_Channel class
			...
			$this->db->query_write("
				...
				VALUES
				('" . $this->db->escape_string($newprefix) . "', '" . $this->db->escape_string(preg_quote($newprefix)) . "(?:/page(?P<pagenum>[0-9]+))?', 'vB5_Route_Channel', 'page', 'index', '', '" . serialize(
				array(
			...
			if ($channel['category'] == 0)
			{
				...
				// TODO:
				// This may need to a check for conversation vs article route in the future.
				// Just putting in a minimal-change fix for now, VBV-11356
				$regex = $this->db->escape_string(preg_quote($newprefix) . '/' . vB5_Route_Conversation::REGEXP);
				$this->db->query_write("
					REPLACE INTO `" . TABLE_PREFIX . "routenew`
					(`prefix`, `regex`, `class`, `controller`, `action`, `template`, `arguments`, `contentid`, `guid`)
					VALUES
					('" . $this->db->escape_string($newprefix) . "', '$regex',
	...
	function step_15($data = null)
	{
		...
		if ($startat == 0)
		{
			$this->show_message($this->phrase['final']['import_themes']);
		...
		catch(vB_Exception_AdminStopMessage $e)
		{
			...
			$message = $this->stop_exception($e);
			$this->show_message($message);
			$this->show_message($this->phrase['core']['skipping_try_again']);
			return;
		}
