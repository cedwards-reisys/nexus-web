...
//				$strreplace[] = $smilie['smilietext'];
	...
	*/
	protected function cleanupBbcode($text)
	{
		...
		if (preg_match_all('#(?<=^|\s)@(?P<username>[^\s@][^@]{' . $min . ',' . $max . '}[^@])#si', $text, $matches))
		{
			$usernames = array();
			...
			{
				if(in_array(substr($username, -1, 1), array(',', '.', ';', '?', '!', '-'), true))
				{
			...
			foreach ($matches['username'] AS $idx => $match)
			{
				...
				if (substr($match, 0, 1) == '"' AND ($pos = strpos($match, '"', 1)) > 1)
				{
					...
					{
						if (strtolower($user['username']) == $tempmatch)
						{
		...
		if (preg_match_all('#\[USER\](.*)\[/USER\]#siU', $text, $matches, PREG_SET_ORDER))
		{
			$usernames = array();
