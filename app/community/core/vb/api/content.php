 ...
 * @version $Id: content.php 85102 2015-07-21 23:13:25Z jinsoojo.ib $
 * @access public
 */
			...
			case vB_Api_Content::ACTION_VIEW:
				...
				foreach ($nodes as $key => $node)
				{
					...
					if (!$node['showpublished'] AND
						!(
							$userContext->getChannelPermission('moderatorpermissions', 'candeleteposts', $node['nodeid'], false, $node['parentid'])
							OR $userContext->getChannelPermission('moderatorpermissions', 'canremoveposts', $node['nodeid'], false, $node['parentid'])
						)
					)
					{
						return false;
