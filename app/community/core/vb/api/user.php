...
//		));
	...
	public function resetPassword($userid, $activationid)
	{
		...
		$maildata = vB_Api::instanceInternal('phrase')->fetchEmailPhrases(
			'resetpw', array($userinfo['username'], $newpassword, $vboptions['frontendurl'], $vboptions['bbtitle']),
			array($vboptions['bbtitle']), $userinfo['languageid']
		);
		vB_Mail::vbmail($userinfo['email'], $maildata['subject'], $maildata['message'], true);
