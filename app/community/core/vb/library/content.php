 ...
 * @version $Id: content.php 84852 2015-05-22 16:40:34Z dgrove $
 * @access public
 */
abstract class vB_Library_Content extends vB_Library
{
	...
	*/
	...
	public function add($data, array $options = array())
	{
		...
		// for the others, this is a no-op. This needs to be called here
		// from the parent class (instead of the text library) to call it
		// before calling afterAdd VBV-14022
		$attachments = $this->handleAttachments('add', $nodeid, $data, $options);
	...
	protected function sendModeratorNotification($nodeid)
	{
		...
		if ($notify === false)
		{
			...
			if ($modQry->valid())
			{
				...
				{
					$events[$mod['userid']] = 'userChg_' . $mod['userid']; //Every time a moderator changes emails, is deleted, etc, we have to invalidate it.
...
";*/
	...
	public function assembleContent(&$content)
	{
		...
		foreach($content AS $key => $record)
		{
			...
			$record['moderate']['candeleteposts'] = $thisChannelPerms['moderate']['canundeleteposts'] = 0;	// TODO: figure out what this does and remove if it does nothing
			...
			if ($thisChannelPerms['canmoderateposts'])
			{
				...
				$record['moderate']['canmoderateposts'] = $thisChannelPerms['moderate']['canmoderateposts'] = 1;	// TODO: figure out what this does and remove if it does nothing
			}
			else
			{
