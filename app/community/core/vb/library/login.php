...
class vB_Library_Login extends vB_Library
{
	 ...
	 *
	 *  Will look up the user's login info from the username and verify that the password is correct
	 *
	 *  @see verifyPasswordFromInfo
	 *  @param $username string.  The name of the user to verify
	 *  @param $passwords array.  Array of password variants in the form
	 *  	array($password, $encoding)
	 ...
	 *  	* userid int id of the user authenticated (null if not authenticated)
	 *  	* remembermetoken string token to use for remember me logic (blank if not authenticated)
	 */
	public function verifyPassword($username, $passwords)
	{
		$db = vB::getDBAssertor();
		$login = $db->getRow('user', array(
			vB_Db_Query::COLUMNS_KEY => array('userid', 'token', 'scheme'),
			'username' => $username
		));
		...
		$auth = $this->verifyPasswordFromInfo($login, $passwords);
		$auth['userid'] = ($auth['auth'] ? $login['userid'] : null);
		return $auth;
	}
