...
if ($_REQUEST['do'] == 'edit' OR $_REQUEST['do'] == 'add')
{
	...
	if ($vbulletin->GPC['userid'])
	{
		...
		{
			print_form_header('./user', 'emailcode', 0, 0);
		...
		{
			$quicklinks[
				"../register.php?" . vB::getCurrentSession()->get('sessionurl') . "do=requestemail&email=" . urlencode(unhtmlspecialchars($user['email'])) . '&amp;url=' . urlencode($vbulletin->options['bburl'] . '/admincp/user.php?do=edit&u=' . $vbulletin->GPC['userid'])
				...
				if ($vbulletin->GPC['send_validated'])
				{
					if (!isset($evalemail_validated["$user[languageid]"]))
					{
						...
						$evalemail_deleted["$user[languageid]"] = vB_Api::instanceInternal('phrase')
							->fetchEmailPhrases('moderation_validated', array(vB5_Route::buildUrl($forumHome['routeid'] . '|fullurl'), $username, $vboptions['bbtitle'], $vboptions['bburl']), array($vboptions['bbtitle']), $chosenlanguage);
					}
