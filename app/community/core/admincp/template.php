...
define('CVS_REVISION', '$RCSfile$ - $Revision: 84866 $');
...
{
	$cssfile = $vb5_options['bburl'] . vB_Api::instanceInternal('style')->fetchCssLocation() . str_pad($vbulletin->options['styleid'], 5, '0', STR_PAD_LEFT) . $vbulletin->stylevars['textdirection']['string'][0] . '/' . 'stylegenerator.css';
}
else
{
...
{
	$stylegeneratorjs .= '
		<link rel="stylesheet" href="'. $vb5_options['bburl'] . '/clientscript/codemirror/lib/codemirror.css?v=' . SIMPLE_VERSION . '">
		<link rel="stylesheet" href="'. $vb5_options['bburl'] . '/clientscript/codemirror/addon/hint/show-hint.css?v=' . SIMPLE_VERSION . '">';
...
if ($_REQUEST['do'] == 'confirmoverwrite')
{
	...
	if ($vbulletin->GPC['require_reupload'])
	{
		...
		echo "<p>&nbsp;</p><p>&nbsp;</p>";
		print_form_header('template', 'upload', 1, 1, '', '75%');
...
if ($_REQUEST['do'] == 'addstyle')
{
	...
	print_form_header('template', 'insertstyle', true);
...
if ($_REQUEST['do'] == 'editstyle')
{
	...
	print_form_header('template', 'updatestyle', true);
	construct_hidden_code('dostyleid', $vbulletin->GPC['dostyleid']);
...
<tr>
	...
	if (vB::getUserContext()->hasAdminPermission('canadmintemplates'))
	{
		echo "		<input type=\"button\" class=\"button\" tabindex=\"1\" value=\"" . $vbphrase['search_in_templates_gstyle'] .
			"\" onclick=\"window.location='template.php?" . vB::getCurrentSession()->get('sessionurl_js') . "do=search';\" />\n";
	}
?>
	</td>
</tr>
